...
.batt-bar.on{ background:#4cff4c; }
.batt-bar.warn{ background:#ffaa00; }
.batt-bar.low{ background:#ff3333; }
.batt-bar:nth-child(1){ height:8px; }
.batt-bar:nth-child(2){ height:12px; }
.batt-bar:nth-child(3){ height:16px; }
.batt-bar:nth-child(4){ height:20px; }
    ...
    <div id="hud-overlay">
        ...
        <div class="hud-row">
            ...
            <div class="hud-box">
                ...
                <div class="hud-title" id="status">BLE LINK</div>
            </div>
        ...
        <div class="hud-row">
            ...
            <div class="hud-box">
                ...
                <div class="hud-value" style="display:flex;align-items:center;justify-content:center;">
                    <span id="hud_volt">0.0</span>
                    <span class="small-unit">V</span>
                    <span class="batt-bars" id="batt-bars">
                        <span class="batt-bar" id="bbar1"></span>
                        <span class="batt-bar" id="bbar2"></span>
                        <span class="batt-bar" id="bbar3"></span>
                        <span class="batt-bar" id="bbar4"></span>
                    </span>
                </div>
            </div>
...
<div id="settings-page">
    ...
    <div id="settings-body">
        ...
        <div class="settings-section-title" style="margin-top:8px;">Screen &amp; Lock</div>

        <button class="compliance-btn" id="btn-wakelock" onclick="toggleWakeLock()">
            &#9728; Keep Screen On: OFF
        </button>

        <button class="compliance-btn" id="btn-autolock" onclick="toggleAutoLock()">
            &#128274; Auto-Lock When Stopped: OFF
        </button>
...
function startGPS(){
    ...
    navigator.geolocation.watchPosition(
        ...
        function(pos){
            ...
            /* SPEED - only update display if above noise floor (0.5 mph) */
            var displayMph = mph >= 0.5 ? mph : 0;
            ...
            if(mph > maxSpeed && mph >= 0.5)
                maxSpeed = mph;

            handleAutoLock(displayMph);
...
function updateBattBars(v){
    var MIN=40, MAX=55;
    var pct = Math.max(0, Math.min(1, (v - MIN) / (MAX - MIN)));
    var filled = Math.round(pct * 4); // 0-4 bars
    var colorClass = pct > 0.5 ? 'on' : pct > 0.25 ? 'warn' : 'low';
    for(var i=1;i<=4;i++){
        var bar = document.getElementById('bbar'+i);
        bar.className = 'batt-bar';
        if(i <= filled) bar.classList.add(colorClass);
    }
}
    ...
    while(true){
        ...
        try{
            ...
            if(txt){
                ...
                // confirmed unlocked
                if(txt.indexOf('system unlocked') !== -1){
                    systemLocked = false;
                    var ub = document.getElementById('lkulBtn');
                    var ul = document.getElementById('lkul-status');
                    ub.textContent = 'UL';
                    ub.classList.remove('locked');
                    ub.classList.add('unlocked');
                    ul.textContent = 'Unlocked';
                }

                // confirmed locked
                if(txt.indexOf('system locked') !== -1 || txt.indexOf('system has been locked') !== -1){
                    systemLocked = true;
                    var lb = document.getElementById('lkulBtn');
                    var ll = document.getElementById('lkul-status');
                    lb.textContent = 'LK';
                    lb.classList.add('locked');
                    lb.classList.remove('unlocked');
                    ll.textContent = 'Locked';
                }
            }
...
/* Send a raw text command via BLE (no input box needed) */
async function sendCmd(txt){
    if(!writeChar){
        log("sendCmd: not connected");
        return;
    }
...
/* LK/UL toggle â€” sends BLE cmd, color changes only on VESC confirmation */
var systemLocked = false;
document.getElementById('lkulBtn').onclick = async function(){
    if(!writeChar){
        log("Not connected");
        return;
    }
    if(systemLocked){
        // currently locked (red) â†’ send unlock
        await sendCmd('ul Calibike');
        log("Unlock cmd sent â€” waiting for confirmation...");
    } else {
        // currently unlocked (green) â†’ send lock
        await sendCmd('lk');
        log("Lock cmd sent â€” waiting for confirmation...");
    }
};
...
var autoLockEnabled = localStorage.getItem('autoLockEnabled') === 'true';
...
async function toggleWakeLock(){
    ...
    var btn = document.getElementById('btn-wakelock');
    if(wakeLockEnabled){
...
/* Called from GPS speed updates */
function handleAutoLock(mph){
    if(!autoLockEnabled) return;
    if(mph < 0.5){
        // stopped â€” start timer if not already running
        if(!autoLockTimer && !systemLocked){
            autoLockTimer = setTimeout(async function(){
                systemLocked = true;
                ...
                log("Auto-locked: bike stopped");
                autoLockTimer = null;
                // send lock command via BLE
                await sendCmd('ul Calibike enable');
            }, 30000); // 30 seconds
        }
    } else {
        // moving â€” cancel pending auto-lock
        if(autoLockTimer){
...
function toggleAutoLock(){
    ...
    var btn = document.getElementById('btn-autolock');
    if(autoLockEnabled){
        btn.textContent = '\uD83D\uDD12 Auto-Lock When Stopped: ON';
        btn.classList.add('active');
        log("Auto-lock enabled (locks after 30s stopped)");
        // enable password system on VESC
        sendCmd('ul Calibike enable');
    } else {
        btn.textContent = '\uD83D\uDD12 Auto-Lock When Stopped: OFF';
        btn.classList.remove('active');
        if(autoLockTimer){ clearTimeout(autoLockTimer); autoLockTimer = null; }
        log("Auto-lock disabled");
        // disable password system on VESC
        sendCmd('ul Calibike disable');
    }
}
...
(function(){
    if(wakeLockEnabled){
        document.getElementById('btn-wakelock').textContent = '\u2600 Keep Screen On: ON';
        document.getElementById('btn-wakelock').classList.add('active');
        requestWakeLock();
    }
    if(autoLockEnabled){
        document.getElementById('btn-autolock').textContent = '\uD83D\uDD12 Auto-Lock When Stopped: ON';
        document.getElementById('btn-autolock').classList.add('active');
    }
})();
